...
public interface PidCouponsRepository extends JpaRepository<CouponEntity, Long> {
	...
	@Query("SELECT new com.javapid.entity.nivo.bar.NivoBarDataByMonth(month,SUM(adults),SUM(seniors),SUM(junior),SUM(students),SUM(portable), SUM(children)) FROM CouponEntity WHERE type IN :sellType AND validity IN :validity AND month IN :months AND year IN :year GROUP BY code,month ORDER by code ASC")
	List<NivoBarDataByMonth> getNivoBarData(@Param("validity") Collection<String> queryType,
	                                        @Param("sellType") Collection<String> sellType,
	                                        @Param("months") Collection<String> months,
	                                        @Param("year") Collection<Integer> year);

	@Query("SELECT new com.javapid.entity.nivo.bar.NivoBarDataByMonth(month,SUM(adults),SUM(seniors),SUM(junior),SUM(students),SUM(portable), SUM(children)) FROM CouponEntity WHERE type IN :sellType AND validity = :validity AND month IN :months AND year IN :year GROUP BY code,month ORDER by code ASC")
	List<NivoBarDataByMonth> getNivoBarDataByValidity(@Param("validity") String queryType,
	                                                  @Param("sellType") Collection<String> sellType,
	                                                  @Param("months") Collection<String> months,
	                                                  @Param("year") Collection<Integer> year);

	@Query("SELECT new com.javapid.entity.nivo.bar.NivoBarDataSumDTO(SUM(adults), SUM(seniors), SUM(junior), SUM(students), SUM(portable), SUM(children)) FROM CouponEntity WHERE type IN :sellType AND validity IN :validity AND month IN :months AND year IN :year GROUP BY year")
